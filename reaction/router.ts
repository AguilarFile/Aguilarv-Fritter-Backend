...
router.post(
    ...
    async (req: Request, res: Response) => {
        const oldReaction = await ReactionCollection.findOne(req.session.userId, req.params.freetId, req.params.type);
 ...
 router.delete(
    ...
    async (req: Request, res: Response) => {
        await ReactionCollection.deleteOne(req.session.userId, req.params.freetId, req.params.type)
        res.status(200).json({
